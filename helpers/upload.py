...
def do_upload(file_to_upload, filename=None):
    try:
        # if file_to_upload is filestorage
        if hasattr(file_to_upload, 'filename'):
            # create file name
            name = filename or "".join(random.choice(string.ascii_lowercase + string.digits) for _ in range(10)) + '.' + file_to_upload.filename.split('.')[-1]
            # upload file
        ...
        # if file_to_upload is a bytes object
        else:
            # create file name
            name = filename or "".join(random.choice(string.ascii_lowercase + string.digits) for _ in range(10)) + '.pdf'
            # upload file
            client.put_object(Bucket=os.environ.get('SPACE_NAME'), Key=name, Body=file_to_upload, ACL='public-read')
            return f"{os.environ.get('SPACE_EDGE_ENDPOINT')}/{os.environ.get('SPACE_NAME')}/{name}"
